...
<body>
  ...
  <script>
    (function () {
      var wallet = ${serializeForInlineScript(walletAddress)};
      var xUsername = ${serializeForInlineScript(xUsername)};
      var claimUrl = ${serializeForInlineScript(claimUrl)};
      var officialWebUrl = ${serializeForInlineScript(officialWebUrl)};
      var twitterAppUrl = ${serializeForInlineScript(twitterAppUrl)};
...
export default async function handler(req, res) {
  try {
    ...
    const walletAddress = String(rawWalletAddress).trim().toLowerCase();
